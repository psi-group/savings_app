...
const Navbar = (props) => {
    ...
    function getName(token) {
        ...
        return JSON.parse(window.atob(token.split('.')[1]))["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier"]
    }
    ...
    /*useEffect(() => {
        setIsRestaurant(useIsRestaurant());
    }, []);*/
    ...
    return (
        <>
            ...
            {useValidateJWT() ?
                ...
                <div>
                        <button onClick={(e) => navigate("/profile")}>Hello {getName(localStorage.getItem("token"))}</button>
                    </div>
                    {!isRestaurant ?
                        ...
                        <div>
                            <button onClick={(e) => navigate("/addProduct")}>Sell Product</button>
                        </div>}
                </> :
                <div >
                    <Link className="text-black top-3 absolute right-3" to="/register">Login/Sign Up</Link>
                </div>
                }
    </>
  );
